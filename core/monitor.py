...
class AirQualityMonitor:
    def __init__(self, pm25_threshold=config.DEFAULT_PM25_THRESHOLD):
        ...
        # Runtime Data
        self.motor_runtime_sec = 1440  # Default: 24 minutes
        self.filter_runtime_sec = 1500  # Default: 25 minutes
        ...
        if os.path.exists(self.runtime_file):
            try:
                with open(self.runtime_file, "r") as f:
                    data = json.load(f)
                    self.motor_runtime_sec = data.get("motor_runtime_sec", 1440)
                    self.filter_runtime_sec = data.get("filter_runtime_sec", 1500)
